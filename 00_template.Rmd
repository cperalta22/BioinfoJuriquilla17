...
columnas <- colnames(pharmacoData)
for ( i in 1:ncol(pharmacoData)){
  cat("\nnumber of", columnas[i], ":", length(unique(pharmacoData[,i])))
}
...
```{r q2}
CCLE_conc <- subset(pharmacoData, pharmacoData$study =="CCLE")
CCLE_conc <- unique(CCLE_conc$concentration)
GDSC_conc <- subset(pharmacoData, pharmacoData$study!="CCLE")
GDSC_conc <- unique(GDSC_conc$concentration)
cat("CCLE concentrations:\n", CCLE_conc, "\nCCLE total:", length(CCLE_conc))
cat("\nGDSC concentrations:\n", GDSC_conc, "\nGDSC total:", length(GDSC_conc))
```
...
```{r q3}
CCLE_V <- subset(pharmacoData, pharmacoData$study =="CCLE")
CCLE_V <- (CCLE_V$viability)
GDSC_V <- subset(pharmacoData, pharmacoData$study!="CCLE")
GDSC_V <- (GDSC_V$viability)
hist(GDSC_V)
hist(CCLE_V)
```
...
cat("\nCCLE range:", range(ccle_viab$viability))
